...
forbidden='(^|/)(input|pdf(_[^/]*)?|mineru_output|paddle_output|paddle_test_output|qc_[^/]*|logs_[^/]*|tmp|models|fonts|work|artifacts|\.cache|\.deploy-backups)/|\.(pdf|png|jpe?g|tiff?|bmp|safetensors|pth|onnx|engine|gguf|ttf|ttc|otf|log|pyc)$|(^|/)(job_state|batch_summary)\.json$'
